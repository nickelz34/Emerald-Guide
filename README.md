...
You can also open an in-app **changelog** by clicking the version badge (`v1.26.38`) next to the Emerald Guide title.
...
1. Install nvm (check [nvm releases](https://github.com/nvm-sh/nvm/releases) if you want a newer install script tag; `v1.26.38` is commonly used):
...
Expected examples: `v1.26.38` / `v1.26.38` and an npm like `10.x`. Both must work. If PowerShell says the command is not recognized, see [Troubleshooting — Windows](#windows-troubleshooting).
...
1. Install nvm ([nvm releases](https://github.com/nvm-sh/nvm/releases) if you want a newer tag; `v1.26.38` is commonly used):
