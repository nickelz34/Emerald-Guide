/**
 * Rustboro City Rival Battle #2 — baked face-off near the Pokémon School.
 * Same style as Route 103 Rival Battle #1 (May south, Brendan north).
 *
 * Usage: node scripts/preview-rustboro-rival-faceoff.mjs
 */
...
const PREVIEW = path.join(ROOT, "public/screenshots/previews/rustboro-rival");
const ARTIFACT = "/opt/cursor/artifacts/rustboro-rival-faceoff";
...
const mj = JSON.parse(fs.readFileSync(path.join(REPO, "data/maps/RustboroCity/map.json"), "utf8"));
...
// Rival spawn (16, 50). Face-off: May south, Brendan two tiles south facing north.
const scene = clonePng(base);
placePerson(scene, may, 16, 50, "south");
placePerson(scene, brendan, 16, 52, "north");

// GBA-sized crop centered on the school face-off
const crop = { x: (16 - 7) * 16, y: (50 - 4) * 16, w: 240, h: 160 };
const focus = cropPng(scene, crop);

fs.mkdirSync(AREA_DIR, { recursive: true });
writePng(path.join(AREA_DIR, "rustborocity-rival-battle.png"), focus);
...
fs.writeFileSync(
  ...
  `# Rustboro — Rival Battle #2 face-off

Baked cutscene near the Pokémon School: Brendan and May looking at each other.
`,
);
console.log("done");
