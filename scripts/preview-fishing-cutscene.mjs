/**
 * Bake a Dewford Town fishing cutscene: Brendan casting with Old Rod near the
 * fisherman who gifts it. Used by Pregame field Event 3 (pregame-field-3).
 *
 * Requires .calib/pokeemerald.
 *
 * Usage: node scripts/preview-fishing-cutscene.mjs
 */
...
const PREVIEW = path.join(ROOT, "public/screenshots/previews/fishing");
const ARTIFACT = "/opt/cursor/artifacts/fishing-cutscene";
...
function renderLayout(layout, voidRgb = [56, 104, 168]) {
...
/** Brendan/May fishing sheets are 32×32 frames (12 across). */
function placeFishing(dest, sprite, tileX, tileY, facing) {
  // Take-out rod ends on frame 3 (W/E), 7 (N), or 11 (S) — rod cast, waiting for a bite.
  const FRAME = { west: 3, east: 3, north: 7, south: 11 };
  const frame = FRAME[facing] ?? 11;
...
const fisherman = loadIndexedSprite(
  "graphics/object_events/pics/people/fisherman.png",
  "graphics/object_events/palettes/npc_1.pal",
);
const brendanFishing = loadIndexedSprite(
  "graphics/object_events/pics/people/brendan/fishing.png",
  "graphics/object_events/palettes/brendan.pal",
);
const splash = loadIndexedSprite(
  "graphics/field_effects/pics/splash.png",
  "graphics/field_effects/palettes/general_0.pal",
);

const mj = JSON.parse(fs.readFileSync(path.join(REPO, "data/maps/DewfordTown/map.json"), "utf8"));
const layout = layoutById.get(mj.layout);
const base = renderLayout(layout, [56, 104, 168]);
const scene = clonePng(base);

// Old Rod fisherman on Dewford's south shore (map.json local coords).
placePerson(scene, fisherman, 12, 14, "north");
// Player on the harbor shore casting north into pier water (Briney’s landing).
placeFishing(scene, brendanFishing, 14, 11, "north");
// Line splash in the water tile the rod faces (frame 0 of 16×8 sheet).
blitSprite(scene, splash, 14 * 16, 9 * 16 + 4, { frameX: 0, frameW: 16, frameH: 8 });
...
publish("dewfordtown-fishing", cropPng(scene, crop, [56, 104, 168]));

fs.writeFileSync(
  path.join(PREVIEW, "README.md"),
  `# Fishing cutscene

- **dewfordtown-fishing** — Brendan fishing on Dewford’s south shore near the Old Rod fisherman (Pregame field Event 3 / \`pregame-field-3\`).
`,
);
console.log("done");
