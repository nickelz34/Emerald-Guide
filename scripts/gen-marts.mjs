...
/** Entrance pin id → mart id (folder stem without location prefix handled in MART_DEFS). */
...
function sectionLabelFromListName(listName, mapFolder) {
  ...
  const themed = listName.match(/Pokemart_(.+)$/i)?.[1];
  if (floor && themed) {
...
function parseMartScripts(mapFolder, itemIndex) {
  ...
  // Find which list is the "basic" path vs expanded by scanning goto_if_set near pokemart calls.
  const unlockByList = new Map();
  for (const m of text.matchAll(
    /goto_if_set\s+(FLAG_\w+)\s*,\s*(\w+)[\s\S]*?pokemart\s+(\w+)/g,
  )) {
    // The goto jumps TO the expanded script — the first pokemart before that jump uses basic.
    // Simpler: find pairs of pokemart labels and nearby flags.
  }
  // Flag that unlocks expanded: look for goto_if_set FLAG ... Expanded, then pokemart ExpandedList
  for (const m of text.matchAll(/goto_if_set\s+(FLAG_\w+)\s*,\s*\w*Expanded\w*/gi)) {
    ...
    for (const list of expandedList) unlockByList.set(list, FLAG_NOTES[flag] || flag.replace(/^FLAG_/, "").replace(/_/g, " "));
  }
  // Also: pokemart after jumping to expanded label
  for (const m of text.matchAll(
    /(\w*Expanded\w*)::\s*\n\s*pokemart\s+(\w+)/gi,
  )) {
    const flagMatch = text.match(
      new RegExp(`goto_if_set\\s+(FLAG_\\w+)\\s*,\\s*${m[1]}`, "i"),
    );
    if (flagMatch) {
      unlockByList.set(
        m[2],
  ...
  for (const m of text.matchAll(
    ...
    const listName = m[1];
    if (!/pokemart/i.test(listName)) continue;
    const items = [];
    for (const im of m[2].matchAll(/\.2byte\s+(ITEM_\w+)/g)) {
      const rec = itemIndex.get(im[1]);
      if (!rec) {
        items.push({
          const: im[1],
          name: titleCase(im[1].replace(/^ITEM_/, "").replace(/_/g, " ")),
          price: 0,
          description: "",
        });
      } else {
...
function main() {
  ...
  for (const def of MART_DEFS) {
    ...
    for (const mapFolder of def.maps) {
      sections.push(...parseMartScripts(mapFolder, itemIndex));
    }
