/**
 * Rustboro Gym — Roxanne face-off cutscene (same style as rival battles).
 * Roxanne on the podium facing south; Brendan two tiles south facing north.
 *
 * Usage: node scripts/preview-roxanne-gym-faceoff.mjs
 */
...
const PREVIEW = path.join(ROOT, "public/screenshots/previews/roxanne-gym");
const ARTIFACT = "/opt/cursor/artifacts/roxanne-gym-faceoff";
  ...
  for (let row = 0; row < h; row++) {
    for (let col = 0; col < w; col++) {
      ...
      if (sx < 0 || sy < 0 || sx >= src.width || sy >= src.height) {
        // Indoor fill — warm brown matching gym wall void rather than overworld water
        out.data[di] = 72;
        out.data[di + 1] = 48;
        out.data[di + 2] = 40;
        out.data[di + 3] = 255;
...
const mj = JSON.parse(fs.readFileSync(path.join(REPO, "data/maps/RustboroCity_Gym/map.json"), "utf8"));
const layout = layoutById.get(mj.layout);
const base = renderLayout(layout);

const roxanne = loadIndexedSprite(
  "graphics/object_events/pics/people/gym_leaders/roxanne.png",
...
// Roxanne stands at (5, 2) FACE_DOWN. Face-off: leader south, player two tiles south facing north.
const scene = clonePng(base);
placePerson(scene, roxanne, 5, 2, "south");
placePerson(scene, brendan, 5, 4, "north");

// GBA-sized crop centered on the podium face-off (gym is only 176px wide)
const crop = { x: (5 - 7) * 16, y: 0, w: 240, h: 160 };
const focus = cropPng(scene, crop);

fs.mkdirSync(AREA_DIR, { recursive: true });
writePng(path.join(AREA_DIR, "rustborocity-gym-roxanne-battle.png"), focus);
for (const dir of [PREVIEW, ARTIFACT]) {
  writePng(path.join(dir, "event-roxanne-faceoff.png"), focus);
  writePng(path.join(dir, "event-roxanne-faceoff@2x.png"), scaleNearest(focus, 2));
}
fs.mkdirSync("/opt/cursor/artifacts/screenshots", { recursive: true });
writePng(
  path.join("/opt/cursor/artifacts/screenshots", "roxanne-gym-faceoff@2x.png"),
  scaleNearest(focus, 2),
);
fs.writeFileSync(
  path.join(PREVIEW, "README.md"),
  `# Rustboro Gym — Roxanne face-off

Baked cutscene on the gym podium: Brendan and Roxanne looking at each other before Gym Battle #1.
`,
);
console.log("done");
