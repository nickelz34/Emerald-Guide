...
const GYM_SPRITE_DIR = path.join(ROOT, "public/sprites/gym");
...
const OUT_BADGES_TS = path.join(ROOT, "src/data/gymBadgesGenerated.ts");
...
// ---- gym badge sprites (trainer card sheet, 8 × 16×16) ----
const BADGE_REL = "graphics/trainer_card/badges.png";
const badgeLocal = path.join(REPO, BADGE_REL);
fs.mkdirSync(GYM_SPRITE_DIR, { recursive: true });
const badgeOut = path.join(GYM_SPRITE_DIR, "badges.png");
if (fs.existsSync(badgeLocal)) {
  fs.copyFileSync(badgeLocal, badgeOut);
} else {
  const buf = await fetchBuffer(`${RAW}/${BADGE_REL}`);
  fs.writeFileSync(badgeOut, buf);
}
keyOutTransparency(badgeOut);

const badgeByMapPoint = {};
for (const m of gymDataTs.matchAll(/mapPointId:\s*"([^"]+)"[\s\S]*?gymNumber:\s*(\d+)/g)) {
  badgeByMapPoint[m[1]] = +m[2] - 1;
}

const badgeTs = `/** Auto-generated by scripts/sync-gym-sprites.mjs — do not edit by hand. */
...
export const GYM_BADGE_FRAME_BY_MAP_POINT: Record<string, number> = ${JSON.stringify(badgeByMapPoint, null, 2)};
...
fs.writeFileSync(OUT_BADGES_TS, badgeTs);
console.log(`Wrote ${OUT_BADGES_TS}`);
