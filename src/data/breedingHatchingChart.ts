import type { BreedingChartSpec } from "./breedingChartTypes";

/** Ch. 2 Event 6 — hatching, Ditto anchor, and story eggs. */
export const HATCHING_BREEDING_CHART: BreedingChartSpec = {
  title: "Hatching & breeding resources",
  lead: "Eggs hatch from steps in your party. Flame Body and Magma Armor do not speed hatching in Gen III — plan long walks or stack multiple Eggs.",
  ariaLabel: "Hatching and breeding resources chart",
  groups: [
    {
      name: "Eggs hatch from steps in your party",
      type: "pairs",
      pairs: [
        {
          parentA: { name: "Mudkip", dex: 258, subtitle: "♀" },
          parentB: { name: "Mudkip", dex: 258, subtitle: "♂" },
          methodLabel: "Egg",
          offspring: [{ name: "Mudkip", dex: 258, subtitle: "~5,355 steps" }],
          note: "Flame Body and Magma Armor do not speed hatching in Gen III. Stack up to five Eggs when grinding long routes or Battle Frontier runs.",
        },
      ],
    },
    {
      name: "Story & post-game breeding anchors",
      type: "grid",
      grid: [
        { name: "Wynaut", dex: 360 },
        { name: "Ditto", dex: 132 },
        { name: "Beldum", dex: 374 },
        { name: "Chikorita", dex: 152 },
        { name: "Cyndaquil", dex: 155 },
        { name: "Totodile", dex: 158 },
      ],
      gridNote:
        "Lavaridge NPC gives a Wynaut Egg before Route 117. Desert Underpass (post-game) is the reliable Ditto spot. Steven's house gifts one Johto starter for breeding dex lines.",
    },
    {
      name: "Useful held items while breeding",
      type: "pairs",
      headerIconName: "Everstone",
      headerIconPath: "sprites/items/icons/everstone.png",
      pairs: [
        {
          parentA: { name: "Ditto", dex: 132, subtitle: "holds Everstone" },
          parentB: { name: "Absol", dex: 359 },
          itemIconName: "Everstone",
          itemIconPath: "sprites/items/icons/everstone.png",
          methodLabel: "Egg",
          offspring: [{ name: "Absol", dex: 359 }],
        },
        {
          parentA: { name: "Marill", dex: 183, subtitle: "holds Sea Incense" },
          parentB: { name: "Ditto", dex: 132 },
          itemIconName: "Sea Incense",
          itemIconPath: "sprites/items/icons/sea_incense.png",
          methodLabel: "Egg",
          offspring: [{ name: "Azurill", dex: 298 }],
        },
      ],
    },
  ],
};
