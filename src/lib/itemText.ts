/**
 * Normalize pokeemerald item/berry text for display in the guide.
 */
...
export function formatItemDescription(desc: string): string {
  ...
  return desc
    ...
    .replace(/\s+/g, " ")
    .trim()
    .replace(/to grow ([A-Z][A-Z0-9]+)\./g, (_, berry: string) => {
...
/** Title-case / normalize item names from game data. */
